...
#reader = csv.reader(csvfile, delimiter=';')
    ...
    for row in reader:
      if k > 0:
        ...
        yaml_text += "name: " + name  + "\n"
        yaml_text += "university: " + row[2].encode('UTF-8').decode('iso-8859-1')  + "\n"
        yaml_text += "date: " + daymonthyear + "\n"
        yaml_text += "aboutme: " +  row[3].encode('UTF-8').decode('iso-8859-1').replace("\n","<br/>")  + "\n"
        yaml_text += "from: " + row[4].encode('UTF-8').decode('iso-8859-1') + "\n"
        yaml_text += "research_topic: " +  row[5].encode('UTF-8').decode('iso-8859-1')  + "\n"
        yaml_text += "abstract: " + row[6].encode('UTF-8').decode('iso-8859-1').replace("\n","<br/>")  + "\n"
        yaml_text += "advisor: "  + row[7].encode('UTF-8').decode('iso-8859-1')  + "\n"
        yaml_text += "keywords: " + row[8].encode('UTF-8').decode('iso-8859-1')  + "\n"
        website = row[9].encode('UTF-8').decode('iso-8859-1')
        if website.lower().find('url') == "-1":
            website = "http://" + website
        yaml_text += "website: "  +   website + "\n"
        img = row[10].encode('UTF-8').decode('iso-8859-1')
        if img == "":
            img = "no_picture.jpg"
        email = row[11].encode('UTF-8').decode('iso-8859-1')
